...
export const BetaIssueReporter: React.FC = () => {
  ...
  const submit = async () => {
    ...
    setStatus('sending');
    try {
      await api.reportBetaIssue({ user_id: getUserId(), category, message: message.trim(), page: location.pathname });
  ...
  return <div className="beta-reporter">
    ...
    {open && <div className="beta-report-backdrop" role="presentation" onClick={()=>setOpen(false)}><section role="dialog" aria-modal="true" aria-labelledby="beta-report-title" onClick={event=>event.stopPropagation()}>
      ...
      <textarea autoFocus maxLength={800} value={message} onChange={e=>setMessage(e.target.value)} placeholder={tr(lang,'Опиши коротко — страница добавится автоматически.','Beschreibe es kurz – die Seite wird automatisch ergänzt.','Describe it briefly — the page is added automatically.')} />
