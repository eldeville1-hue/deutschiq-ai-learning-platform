...
export const Result: React.FC = () => {
  ...
  const pillars = [
    ...
    ['pronunciation', tr(lang, 'Образец речи', 'Sprechprobe', 'Speaking sample')],
  ];
  const percent = (value: any) => Math.round(Number(value || 0) * (Number(value || 0) <= 10 ? 10 : 1));
  return (
    <main className="app-shell result-page page-enter">
      <header className="result-hero"><span className="result-icon"><FaCheck /></span><p className="eyebrow">{tr(lang, 'Предварительная оценка уровня', 'Vorläufige Niveaueinschätzung', 'Preliminary level estimate')}</p><h1>{data.level || 'A1'}</h1><strong>{Math.round(data.overall_score || 0)}%</strong><p>{tr(lang, 'Это стартовая оценка по ответам в тесте. Говорение оценивается только после образца речи.', 'Das ist eine erste Einschätzung aus deinen Testantworten. Sprechen wird erst nach einer Sprechprobe bewertet.', 'This is a starting estimate from your test answers. Speaking is assessed only after a speech sample.')}</p><small>{tr(lang, `Уверенность: ${data.confidence === 'high' ? 'высокая' : data.confidence === 'medium' ? 'средняя' : 'низкая'}`, `Sicherheit: ${data.confidence === 'high' ? 'hoch' : data.confidence === 'medium' ? 'mittel' : 'niedrig'}`, `Confidence: ${data.confidence || 'low'}`)}</small></header>
