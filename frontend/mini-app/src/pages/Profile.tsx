...
export const Profile: React.FC = () => {
  ...
  return (
    <main className="app-shell profile-page precision-profile v30-page v30-profile page-enter">
      ...
      <section className="profile-learning-pass page-stagger-2"><header><small>{tr(lang, 'НА ЭТОЙ НЕДЕЛЕ', 'DIESE WOCHE', 'THIS WEEK')}</small></header><h2>{completed ? tr(lang, `${completed} уроков завершено`, `${completed} Lektionen abgeschlossen`, `${completed} lessons completed`) : tr(lang, 'Первый навык уже готов', 'Das erste Lernziel ist bereit', 'Your first skill is ready')}</h2><p>{tr(lang, `${data.streak || 0} дней подряд · продолжим без давления`, `${data.streak || 0} Tage in Folge · entspannt weitermachen`, `${data.streak || 0} day streak · continue at your pace`)}</p><button type="button" onClick={() => navigate(withUser('/plan'))}>{tr(lang, 'Продолжить обучение', 'Weiterlernen', 'Continue learning')} <FaArrowRight /></button><div><span><b><CountUp value={data.xp || 0} /></b> XP</span><span><b>{activityCount}</b> {tr(lang, 'активностей', 'Aktivitäten', 'activities')}</span></div></section>
