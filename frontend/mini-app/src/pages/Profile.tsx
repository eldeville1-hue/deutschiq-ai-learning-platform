...
export const Profile: React.FC = () => {
  ...
  const exportData = async () => {
    try {
      ...
      setActionStatus(lang === 'ru' ? 'Файл готов.' : 'Datei ist bereit.');
    } catch { setActionStatus(lang === 'ru' ? 'Не удалось скачать данные.' : 'Daten konnten nicht geladen werden.'); }
  };
  ...
  return (
    <main className="app-shell profile-page precision-profile v30-page v30-profile page-enter">
      ...
      <section className="subscription-card active beta-access"><header><span><FaCheck /> {lang === 'ru' ? 'БЕСПЛАТНАЯ БЕТА' : 'KOSTENLOSE BETA'}</span><b>{lang === 'ru' ? 'ОТКРЫТО' : 'OFFEN'}</b></header><h2>{lang === 'ru' ? 'Все функции доступны' : 'Alle Funktionen verfügbar'}</h2><p>{lang === 'ru' ? 'Тестируй приложение бесплатно.' : 'Teste die App kostenlos.'}</p></section>
