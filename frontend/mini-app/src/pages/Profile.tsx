...
import { FaArrowRight, FaCheck, FaChevronRight, FaCommentDots, FaDownload, FaMedal, FaMoon, FaPaperPlane, FaRedo, FaShareAlt, FaSun, FaTrash } from 'react-icons/fa';
...
export const Profile: React.FC = () => {
  ...
  return (
    <main className="app-shell profile-page precision-profile v30-page v30-profile page-enter">
      ...
      <section className="profile-learning-pass page-stagger-2"><header><small>{tr(lang, 'ТВОЙ УЧЕБНЫЙ ПАСПОРТ', 'DEIN LERNPASS', 'YOUR LEARNING PASSPORT')}</small><span>{data.level || 'A1'}</span></header><h2>{completed ? tr(lang, 'Маршрут уже движется', 'Dein Lernweg ist in Bewegung', 'Your learning path is moving') : tr(lang, 'Первый практический навык ждёт', 'Deine erste praktische Fähigkeit wartet', 'Your first practical skill is ready')}</h2><p>{tr(lang, `${completed} уроков завершено · ${data.streak || 0} дней подряд`, `${completed} Lektionen abgeschlossen · ${data.streak || 0} Tage in Folge`, `${completed} lessons completed · ${data.streak || 0} day streak`)}</p><button type="button" onClick={() => navigate(withUser('/plan'))}>{tr(lang, 'Открыть мой маршрут', 'Meinen Weg öffnen', 'Open my path')} <FaArrowRight /></button><div><span><b><CountUp value={data.xp || 0} /></b> XP</span><span><b>{activityCount}</b> {tr(lang, 'активностей', 'Aktivitäten', 'activities')}</span></div></section>
