...
export const Checkpoint: React.FC = () => {
  ...
  if (result) return <main className="app-shell checkpoint-page checkpoint-result"><span className={`result-icon ${result.passed ? '' : 'needs-practice'}`}>{result.passed ? <FaCheck /> : <FaTimes />}</span><p className="eyebrow">{result.passed ? tr(lang, 'УРОВЕНЬ ПОДТВЕРЖДЁН', 'NIVEAU BESTÄTIGT', 'LEVEL CONFIRMED') : tr(lang, 'НУЖНО ЕЩЁ ПОВТОРИТЬ', 'NOCH ETWAS ÜBEN', 'MORE REVIEW NEEDED')}</p><h1>{result.score}%</h1>{!result.passed && result.recovery_topics?.length > 0 && <div className="checkpoint-recovery"><small>{tr(lang, 'ПЕРЕД ПОВТОРНОЙ ПОПЫТКОЙ', 'VOR DEM NÄCHSTEN VERSUCH', 'BEFORE YOUR NEXT ATTEMPT')}</small>{result.recovery_topics.map((topic: string) => <span key={topic}>{topic}</span>)}</div>}{result.unlocked_level && <div className="level-unlocked"><small>{tr(lang, 'ОТКРЫТ НОВЫЙ УРОВЕНЬ', 'NEUES NIVEAU FREIGESCHALTET', 'NEW LEVEL UNLOCKED')}</small><strong>{result.unlocked_level}</strong><span>{tr(lang, 'Новый маршрут уже доступен.', 'Der neue Lernweg ist jetzt verfügbar.', 'Your new learning path is ready.')}</span></div>}<button className="rc-primary" onClick={() => navigate(withUser(result.passed ? '/plan' : '/review'))}>{result.passed ? tr(lang, 'К маршруту', 'Zum Lernweg', 'Go to journey') : tr(lang, 'Начать восстановление', 'Wiederholung starten', 'Start recovery')}</button></main>;
  return <main className="app-shell checkpoint-page"><header><p className="eyebrow">{level} · {tr(lang, 'ФИНАЛЬНЫЙ ТЕСТ', 'ABSCHLUSSTEST', 'FINAL CHECKPOINT')}</p><h1>{index + 1}/{data.items.length}</h1><div className="progress-bar"><div className="progress-bar-fill" style={{ width: `${((index + 1) / data.items.length) * 100}%` }} /></div></header><section className="checkpoint-card"><h2>{item?.question}</h2>{item && <ExerciseInteraction exercise={{ ...item, id: `checkpoint-${level}-${index}` }} answer={answer} onAnswer={setAnswer} lang={lang} />}<button className="rc-primary" disabled={!answer.trim()} onClick={next}>{index + 1 === data.items.length ? tr(lang, 'Завершить', 'Abschließen', 'Finish') : tr(lang, 'Дальше', 'Weiter', 'Next')} <FaArrowRight /></button></section></main>;
};
