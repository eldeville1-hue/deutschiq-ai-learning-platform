  ...
  return (
    <main className="app-shell rc-page rc-analytics page-enter">
      ...
      <div className="rc-insight-grid">
        <details className="rc-analysis-detail"><summary><span><small>{tr(lang, 'ПАМЯТЬ', 'GEDÄCHTNIS', 'MEMORY')}</small><strong>{tr(lang, 'Удержание знаний', 'Wissensspeicherung', 'Knowledge retention')}</strong></span><FaChevronDown /></summary><div className="rc-detail-content">{learning?.mastery?.length ? learning.mastery.slice(0, 4).map((item: any) => <div className="rc-detail-row" key={item.topic}><span><strong>{topicLabel(item.topic, lang)}</strong><small>{item.attempts} {tr(lang, 'попыток', 'Versuche', 'attempts')}</small></span><b>{item.mastery}%</b></div>) : <p>{tr(lang, 'Появится после практики и повторений.', 'Erscheint nach Übungen und Wiederholungen.', 'Appears after practice and review.')}</p>}</div></details>
