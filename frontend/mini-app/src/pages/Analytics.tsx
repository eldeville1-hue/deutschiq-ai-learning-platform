...
export const Analytics: React.FC = () => {
  ...
  return (
    <main className={`app-shell rc-page rc-analytics page-enter level-${String(data?.level || 'a1').toLowerCase()}`}>
      ...
      <button type="button" className="rc-evidence-toggle" onClick={() => setShowEvidence(value => !value)} aria-expanded={showEvidence}>
        <span><small>{tr(lang, 'ПОДРОБНОСТИ', 'DETAILS', 'DETAILS')}</small><strong>{tr(lang, 'Навыки и удержание знаний', 'Fähigkeiten und Behalten', 'Skills and retention')}</strong></span><FaChevronDown className={showEvidence ? 'rotated' : ''} />
      </button>

      {showEvidence && <div className="rc-evidence-panel"><section className="rc-skill-report">
